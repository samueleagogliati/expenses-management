...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id, username: user.username }, 'sassasamu', { expiresIn: '60m' })
...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const newUser = await User.query().insert({firstname: firstname, lastname: lastname, username: username, email:email, password: hashedPassword })
...
router.post('/expenses_of_day', async (req, res) => {
    ...
    const expenses = await Expense.query().withGraphFetched('category').where('date', new Date(selectedDate)).andWhere('user_id', userId)
    res.json(expenses)
})

router.post('/all_expenses', async (req, res) => {
  let { user_id, month, year } = req.body
  let query = Expense.query().withGraphFetched('category').where("user_id", user_id)
...
router.post('/saveExpense', async (req, res) => {
    try {
        ...
        date = new Date(date)
        let newExpense = await Expense.saveData({ category_id, description, price, date, user_id })
...
router.put('/expenses/:id', async (req, res) => {
  try {
      ...
      if (isNaN(id)) {
          return res.status(400).json({ error: 'Expense ID must be a valid number' })
      }
      ...
      const updatedExpense = await Expense.query().patchAndFetchById(id, { category_id, description, price })
...
router.delete('/expenses/:id', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Errore durante l\'eliminazione dell\'expense' });
    }
})
...
router.post('/expenses', async (req, res) => {
  try {
    ...
    const total = await Expense.getTotalExpensesByMonthAndYearAndUser(monthInt, yearInt, userId)
...
router.post('/expenses_of_days', async (req, res) => {
  try {
    ...
    const expenses = await Expense.getExpensesByDayAndUser(monthInt, yearInt, userId)
...
router.post('/expenses_by_category', async (req, res) => {
  try {
    ...
    console.error('Errore durante il recupero delle spese raggruppate per categoria:', error)
    res.status(500).json({ error: 'Errore durante il recupero delle spese raggruppate per categoria' })
  }
})
// END EXPENSES
